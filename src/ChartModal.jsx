  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* STRISCIA TIMELINE INFERIORE (due corsie Output/Input, ultimo elemento colonna Flexbox) */}
        <div style={{ width: '100%', minHeight: '80px', paddingBottom: 'max(env(safe-area-inset-bottom), 20px)', background: '#0a0a12', borderTop: '1px solid #333', position: 'relative', flexShrink: 0 }}>
          <div style={{ position: 'absolute', left: '50px', right: '15px', top: 0, bottom: 0, display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ flex: 1, position: 'relative', minHeight: '40px', borderTop: '1px dashed rgba(255,255,255,0.05)' }}>
              {activeNodesWithStack.filter(n => n.type !== 'workout' && n.type !== 'cognitive' && n.type !== 'sleep').map((node) => {
                const primaryTypes = MODAL_NODE_PRIMARY[expandedChart] ?? NODE_IMPORTANCE[expandedChart] ?? [];
                const isPrimary = primaryTypes.includes(node.type);
                const isWork = node.type === 'work';
