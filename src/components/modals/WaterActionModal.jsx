...
/**
 * Vista drawer "Azioni rapide" — idratazione.
 * Conferma in fixed/portal (sopra overflow-hidden del MenuDrawerShell).
 */
export default function WaterActionModal({
  ...
  const confirmFooter = (
    <div
      className="pointer-events-none fixed bottom-4 left-4 right-4 z-[100050]"
      style={{
    ...
    >
      <button
        type="button"
        className="pointer-events-auto w-full rounded-[15px] border-none px-[18px] py-[18px] text-[0.9rem] font-bold tracking-[2px] shadow-[0_0_20px_rgba(0,229,255,0.35)]"
        style={{
          background: 'linear-gradient(135deg, #0891b2, #00e5ff)',
          color: '#041018',
          cursor: 'pointer',
        }}
        onClick={(event) => {
  ...
  return (
    <div className="view-animate relative flex min-h-0 w-full flex-1 flex-col overflow-hidden pb-24">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain pb-8 [-webkit-overflow-scrolling:touch]">
        ...
        <div className="water-glass" style={{ padding: '16px', display: 'flex', gap: '12px', marginBottom: '12px' }}>
          ...
          >
            ...
            <span style={{ fontSize: '0.85rem', fontWeight: 'bold' }}>+ 250</span>
          </button>
          <button
            type="button"
            onClick={(event) => {
          ...
          >
            <span style={{ fontSize: '1.8rem', marginBottom: '4px' }}>🚰</span>
            <span style={{ fontSize: '0.85rem', fontWeight: 'bold' }}>+ 500</span>
          </button>
        </div>
      ...
      {typeof document !== 'undefined' ? createPortal(confirmFooter, document.body) : confirmFooter}
    </div>
  );
}
