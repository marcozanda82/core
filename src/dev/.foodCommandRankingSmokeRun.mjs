...
function tokenizeFoodName(value) {
  const rawParts = String(value ?? "").toLowerCase().normalize("NFD").replace(ACCENT_REGEX, "").replace(/[''`´]/g, " ").split(/\s+/);
  const out = [];
...
function normalizeComparableFoodString(s) {
  let t = String(s ?? "").normalize("NFD").replace(ACCENT_REGEX, "");
  t = t.replace(/[''`´]/g, "");
...
function buildFoodCommandItem({ rawSegment, foodDb, flatLog, mealContext }) {
  ...
  const candidates = candidatesFull.map((c) => ({
    ...
    score: Math.max(0, Math.round(Math.min(1.5, Math.max(-0.5, c.bm.score)) * 1e4))
  }));
