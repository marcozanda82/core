...
// src/dev/foodCommandRankingSmoke.js
import assert from "node:assert";

// src/coreEngine.jsx
var import_react2 = __toESM(require_react(), 1);

// src/useBiochimico.js
...
var TARGETS = {
  ...
  min: { ca: 1e3, fe: 18, mg: 420, p: 700, k: 3400, na: 2e3, zn: 11, cu: 0.9, se: 55 },
  fat: { fatSat: 20, fatTrans: 2, fatMono: 45, fatPoly: 20, omega3: 2, omega6: 10, colest: 300 }
};
var ALL_NUTRIENT_KEYS = Object.values(TARGETS).flatMap((g) => Object.keys(g));

// src/coreEngine.jsx
var RADIAN = Math.PI / 180;
var MEAL_TYPE_GROUPS = {
  colazione: ["colazione", "merenda1"],
  snack: ["snack", "merenda_am", "merenda_pm", "merenda2", "spuntino"],
  pranzo: ["pranzo"],
  cena: ["cena"]
};
var MEAL_TYPE_TO_CANONICAL = {};
Object.entries(MEAL_TYPE_GROUPS).forEach(([canonical, aliases]) => {
  aliases.forEach((alias) => {
    MEAL_TYPE_TO_CANONICAL[alias] = canonical;
  });
});
var KNOWLEDGE_BASE_MAX_AGE_MS = 3 * 24 * 60 * 60 * 1e3;
var FOOD_CATEGORY_PROTEINA = "proteina";
var FOOD_CATEGORY_CARBO = "carbo";
var FOOD_CATEGORY_GRASSO = "grasso";
var SWAP_CATEGORY_DEFAULT_MACRO_PER100 = {
  [FOOD_CATEGORY_PROTEINA]: 22,
  [FOOD_CATEGORY_CARBO]: 60,
  [FOOD_CATEGORY_GRASSO]: 40
};

// src/features/salaComandi/utils/foodUtils.js
...
function findRecentFoodHabit(query, foodDb, flatLog) {
  ...
  const defaultQty = lastQty != null ? lastQty : Number.isFinite(dq) && dq > 0 ? Math.round(dq) : 150;
  return {
    dbKey: bestKey,
...
// src/features/salaComandi/engines/foodCommandConcepts.js
var ACCENT_REGEX = /[\u0300-\u036f]/g;
var FOOD_CONCEPTS = Object.freeze({
...
var EXTRA_SKIP = /* @__PURE__ */ new Set([
  "e",
  "ed",
  "oppure",
  "o",
...
function stripNorm(s) {
  return String(s ?? "").toLowerCase().normalize("NFD").replace(ACCENT_REGEX, "").replace(/[''`´]/g, " ");
}
function applyFoodPhrases(normalized) {
...
function toConceptTokenList(raw, engineSkipTokens = []) {
  const skip = /* @__PURE__ */ new Set([...engineStopSet(engineSkipTokens), ...EXTRA_SKIP]);
  let s = stripNorm(raw);
  s = applyFoodPhrases(s);
  const rawParts = s.split(/\s+/);
  const out = [];
  for (let i = 0; i < rawParts.length; i += 1) {
    let t = rawParts[i].replace(/^[^a-z0-9àèéìòù]+/gi, "").replace(/[^a-z0-9àèéìòù]+$/gi, "");
...
// src/features/salaComandi/engines/foodCommandEngine.js
var ACCENT_REGEX2 = /[\u0300-\u036f]/g;
var READY_BIDIRECT_MIN = 0.92;
var READY_DESC_COVERAGE_MIN = 0.75;
var AMBIGUITY_BIDIRECT_GAP = 0.055;
var AMBIGUITY_SECOND_MIN_SCORE = 0.72;
var SKIP_TOKENS = [
  "alla",
  "allo",
  "al",
...
function scoreBidirectionalMatch(query, desc) {
  ...
  const lengthPenalty = totalFoodTokens > totalQueryTokens ? (totalFoodTokens - totalQueryTokens) * 0.1 : 0;
...
function normalizeComparableFoodString(s) {
  let t = String(s ?? "").normalize("NFD").replace(ACCENT_REGEX2, "");
  t = t.replace(/[''`´]/g, "");
...
var DERIVED_RANK_PREFIXES = ["farina", "crema", "latte", "biscotti", "pomodori"];
function singleConceptRankingAdjustments(dbName, qTok, dTok) {
  ...
  const startsWithPrimaryConcept = normalizedDesc.startsWith(qt0) || dTok.length > 0 && d0 === qt0;
  let total = 0;
  ...
  const derivedPenalty = DERIVED_RANK_PREFIXES.some((p) => normalizedDesc.startsWith(p)) ? 0.12 : 0;
...
var IT_ARTICLE_ONE = /^(?:un[ao']\s+|un\s+|una\s+|uno\s+)/i;
var NUM_WORD_MAP = Object.freeze({
  un: 1,
  una: 1,
  uno: 1,
  due: 2,
...
function splitFoodCommandText(text) {
  ...
  if (!s) return [];
  let chunks = s.replace(/\r\n/g, "\n").split(/\n|,|;/).map((x) => x.trim()).filter(Boolean);
  chunks = chunks.flatMap(
    (c) => c.split("+").map((x) => x.trim()).filter(Boolean)
  );
  chunks = chunks.flatMap(
    (c) => c.split(/\s+e\s+/i).map((x) => x.trim()).filter(Boolean)
  );
  return chunks;
}
function normalizeFoodText(fragment) {
  return String(fragment || "").trim().replace(/\s+/g, " ");
}
...
function collectFoodCandidates(query, foodDb, max = 8) {
  ...
  for (const key in foodDb) {
    ...
    const dbName = String(item?.desc ?? item?.name ?? "").trim();
    if (!dbName) continue;
    ...
    const secondary = -dLen * 1e-3;
    list.push({
      key,
      sortKey: compositeRankScore + secondary,
      rankAdj: import.meta.env?.DEV ? rankAdj : void 0,
      item,
      bm,
      compositeRankScore
    });
...
function buildFoodCommandItem({ rawSegment, foodDb, flatLog, mealContext }) {
  ...
  const candidates = candidatesFull.map((c) => ({
    ...
    score: Math.max(0, Math.round(Math.min(1.5, Math.max(-0.5, c.score)) * 1e4))
  }));
  ...
  if (!firstByScore) {
    ...
    const gap = secondByScore != null ? firstByScore.bm.score - secondByScore.bm.score : 1;
    const secondDesc = secondByScore ? String(secondByScore.item?.desc ?? secondByScore.item?.name ?? "") : "";
    const secondReadyGate = secondByScore ? passesBidirectReadyGate(secondByScore.bm, nameForMatch, secondDesc) : false;
    if (secondByScore && gap <= AMBIGUITY_BIDIRECT_GAP && secondByScore.bm.score >= AMBIGUITY_SECOND_MIN_SCORE && firstReadyGate && secondReadyGate) {
      itemStatus = "ambiguous";
      confidence = Math.min(0.9, Math.max(0.35, bm1.score + 0.05));
      reason = "Pi\xF9 voci equivalgono al comando: scegli quella corretta.";
    } else if (secondByScore && gap <= AMBIGUITY_BIDIRECT_GAP && bm1.score >= 0.58 && secondByScore.bm.score >= 0.58) {
      itemStatus = "ambiguous";
      ...
      const singleConceptQuery = toConceptTokenList(nameForMatch, SKIP_TOKENS).length === 1;
      if (singleConceptQuery && candidatesFull.length > 1) {
        itemStatus = "ambiguous";
        confidence = Math.min(0.9, Math.max(0.35, bm1.score + 0.05));
        reason = "Pi\xF9 voci plausibili per un termine generico: scegli quella corretta.";
      } else {
        itemStatus = "ready";
        confidence = Math.min(1, Math.max(0.72, bm1.score));
        reason = practicallyEqualFoodLabel(nameForMatch, desc1) ? "Corrispondenza diretta con il database." : "Score di pertinenza alto e sufficiente coincidenza con la descrizione.";
      }
  ...
  const matched = top && itemStatus !== "no_match" && itemStatus !== "ambiguous" ? buildMatchedFoodSnapshot(top.key, top.item) : null;
  const habit = nameForMatch && foodDb && Object.keys(foodDb).length > 0 ? findRecentFoodHabit(nameForMatch, foodDb, flatLog) : null;
  let quantity = null;
  ...
  } else if (extracted.countHint != null && !extracted.countHintFromArticle && itemStatus !== "ambiguous" && (itemStatus === "ready" || itemStatus === "needs_review")) {
    suggestedQuantity = extracted.countHint;
    quantity = extracted.countHint;
    quantitySource = "count_hint";
  } else if (itemStatus !== "ambiguous" && topKey && habit && habit.dbKey === topKey) {
  ...
  if (quantity == null && itemDefaultQty != null && matched && itemStatus !== "ambiguous") {
  ...
  if (quantity == null && extracted.countHint != null && extracted.countHintFromArticle && itemStatus !== "ambiguous" && (itemStatus === "ready" || itemStatus === "needs_review")) {
  ...
  if (import.meta.env?.DEV) {
    ...
    console.log("[foodCommandEngine:DEV:item]", {
      input: displayRawName,
      queryTokens,
      candidates: candidatesFull.slice(0, 5).map((c) => ({
        name: String(c.item?.desc ?? c.item?.name ?? ""),
        score: c.bm?.score,
        rankScore: c.score,
        queryCoverage: c.bm?.queryCoverage,
        foodCoverage: c.bm?.foodCoverage,
        startsWithPrimaryConcept: c.rankAdj?.startsWithPrimaryConcept,
        simplicityBonus: c.rankAdj?.simplicityBonus,
        derivedPenalty: c.rankAdj?.derivedPenalty
      }))
    });
  }
  return {
...
function parseFoodCommandIntent(input = {}) {
  const text = input.text != null ? String(input.text) : "";
  const foodDb = input.foodDb != null && typeof input.foodDb === "object" && !Array.isArray(input.foodDb) ? input.foodDb : {};
  const flatLog = Array.isArray(input.flatLog) ? input.flatLog : [];
  const mealContext = input.mealContext != null && typeof input.mealContext === "object" ? input.mealContext : null;
  const errors = [];
  const debug = {
    segments: (
      /** @type {string[]} */
