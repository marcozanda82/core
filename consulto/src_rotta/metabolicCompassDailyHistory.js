 ...
 * @returns {Array<{ date: string, kcalBalance: number, trainingLoad: number }>}
 */
export function buildMetabolicCompassDailyHistory(
  ...
  for (let i = maxDays - 1; i >= 0; i -= 1) {
    ...
    if (log.length === 0) {
      out.push({ date: dStr, kcalBalance: 0, trainingLoad: 0 });
      continue;
    }
    ...
    out.push({ date: dStr, kcalBalance, trainingLoad });
  }
